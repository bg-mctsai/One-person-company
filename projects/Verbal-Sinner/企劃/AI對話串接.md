...
## 3.1 App Check（上線前必做；MVP 先關掉）

> **MVP 決策：App Check 先關掉**（開發與驗證流程優先）。  
> 但只要你公開上線，**就必須補 App Check**，不然任何人拿到你的 callable 名稱就能刷爆你（你付 AI 錢）。

- Functions 端可設定 `enforceAppCheck: true`（上線前打開）
- Web 端可用 reCAPTCHA v3 / Enterprise 作為 App Check provider（上線前設定）
...
### 6.1 硬防線（上線前必做；MVP 不做）

- **App Check**：擋外部腳本濫用（上線前把 Functions 設為 `enforceAppCheck: true`）
- **token 化（最直覺、最好控成本）**：用「訊息門檻」+「通行證次數」去控制“嘗試次數”，而不是賣一堆商品。  
  - **免訊息通行證**：N 次（每次可抵扣一次「入幕」或一次「重玩該關」）
