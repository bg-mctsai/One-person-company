...
| 多人場景：同事B插話帶風向 | 可選 AI | 0–1 | 做出「每次都不一樣的風向話術」，提升重玩性與壓迫感 | 固定風向模板庫 |
| 多人場景：主管/HR 打斷與流程話術 | 模板為主 | 0–1 | 這類句子要穩、要像模板公文；AI 容易漂風格 | 固定流程話術模板 |
...
- **群體壓迫的變化**：同事B的風向話術每次不完全相同，玩家才不覺得背台詞
...
- **主管**：打斷你的長篇解釋、逼你承諾、要求「結論/時程」
- **HR**：把話鎖回「流程/紀錄/可受理範圍」
- **同事B**：把你的話剪成風向（「你看他又在解釋」「他情緒又來了」）
- **同事A**：假客觀切割（「我不是針對你」）或補刀（把責任推回你）
...
- **主管/HR 的打斷**：優先模板（風格穩、像制度話術）
- **同事B 的打斷**：可用 AI 產生變體（只為不重複），但每幕最多 1 次
