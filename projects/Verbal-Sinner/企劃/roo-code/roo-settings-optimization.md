...
4. **選擇模型（免費層可用選項）**
   - **推薦**：`gemini-flash-latest`（你目前選中的，最佳選擇）⭐
   - **備選**：`gemini-flash-lite-latest`（更輕量，速度更快）
   - **複雜任務**：`gemini-2.5-pro`（免費層可用，但可能有配額限制）
   - **注意**：免費層有每日/每分鐘請求限制，但成本為 $0（在免費額度內）
...
**基本設定：**
- ✅ **模型**：`gemini-flash-latest`（免費層最佳選擇，自動更新到最新穩定版）⭐
...
- **模型**：`gemini-flash-latest`（你目前選中的）⭐
- **成本**：**$0**（免費額度內）
- **配額**：免費層有每日/每分鐘請求限制（通常足夠日常開發）
- **用途**：React 組件、Firebase Functions、日常代碼生成

### 複雜任務（10% 使用）- 免費層
- **Profile**：切換到 `Gemini Pro (複雜)` 或保留 `Claude Sonnet (備用)`
- **模型**：`gemini-2.5-pro`（免費層可用）或 `claude-sonnet-4-5`（付費）
- **成本**：免費層可能為 $0（在配額內），或需付費
- **用途**：理解複雜企劃文件、架構設計

### 成本對比

| 場景 | 目前（Claude） | 優化後（Gemini 免費層） | 節省 |
|------|---------------|----------------------|------|
| 日常開發 | $6.00/$22.50 | **$0**（免費額度內） | **100%** ⭐ |
| 複雜任務 | $6.00/$22.50 | $0（免費額度內）或付費 | **100%** 或 79.2% |
...
- [ ] 已設定 Gemini API key
- [ ] 已選擇 `gemini-flash-latest` 模型（免費層最佳選擇）
...
1. **配額限制**
   - 免費層有每日/每分鐘請求數限制
   - 通常在 Google Cloud Console 可查看配額使用情況
   - 超過配額後可能需要等待或升級到付費層

2. **模型可用性**
   - 免費層可能只有部分模型可用
   - `gemini-flash-latest` 是免費層的最佳選擇
   - Pro 模型可能有更嚴格的配額限制
...
2. **模型切換**
   - `gemini-flash-latest` 適合大部分日常任務
   - 需要更強能力時可切換到 `gemini-2.5-pro` 或 Claude
